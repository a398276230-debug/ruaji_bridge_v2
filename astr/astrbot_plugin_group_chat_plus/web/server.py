...
<h1>访问出错</h1><p>{safe_reason or "请联系管理员。"}</p></div></body></html>"""
    ...
    def _get_config_file_path(self) -> str:
        """获取插件真实配置文件路径（动态定位在统一宿主 data/plugin_data 目录下）"""
        if hasattr(self.plugin.config, "config_path") and self.plugin.config.config_path:
            return self.plugin.config.config_path
        try:
            from astrbot.core.utils.astrbot_path import get_astrbot_plugin_data_path
            return os.path.join(get_astrbot_plugin_data_path(), "astrbot_plugin_group_chat_plus", "config.json")
        except Exception:
            return os.path.join(os.getcwd(), "data", "plugin_data", "astrbot_plugin_group_chat_plus", "config.json")
